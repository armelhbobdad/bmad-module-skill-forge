---
injection:
- heading: 'Most recently recorded, newest first — titles and keys only:'
  filter:
    created:
      $exists: true
  sort: created:-1
...
One document per fact, keyed `memory/<slug>` — a slug of its title under the `memory/` folder (`memory/cache-warmup-order`, not `memory/2026/misc-3`, and never at the root, which is the project's front door). Everything else in the graph — `README`, `CHANGELOG`, `docs/…`, `src/…` — is the repository's own markdown: a note may link one of those by its root-absolute key (`docs/architecture`, `src/knowledge/confidence-tiers`) when the fact is about it, and memory never creates or edits a document outside `memory/`. The `memory` schema in `.iwe/schemas/` binds every key and `--strict` enforces it: a document carrying `created` stamps it `YYYY-MM-DD HH:MM` and keeps `session` a non-empty string; a write that fails is fixed, never retried without the flag. Compose them with `--content -`, the document on stdin via a quoted heredoc:
...
A note migrated from the legacy claude-mem store carries `source: claude-mem`, `source_table` (`observations`, `session_summaries`, or `both`) and `source_ids` (row ids in that store); its `created` is the row's own timestamp and its `session` the claude-mem session id, which has no `.iwe/claude/sessions/` record. `iwe find --filter '{ source: "claude-mem" }' -f keys` lists them all.
