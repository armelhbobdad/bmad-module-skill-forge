...
| **Gates** | step 1: Input Gate [use args] | step 6: Report Menu [R review / X exit] (headless default X) |
...
| 3    | resolution-failure   | step 1 §2 (`{skills_output_folder}` does not exist or is empty); step 1 §3 (forge_data_folder unconfigured); any stage that cannot resolve a required shared helper (atomic-write / schema ref) from its probe order |
...
| 8    | analysis-halted      | coverage.md §7 & integrations.md §7 — user picks [X] at an elective vacuous-analysis gate (interactive only) |
