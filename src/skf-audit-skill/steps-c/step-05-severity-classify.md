...
**Rollup inherits from step-03.** If step-03 §5 collapsed ≥ 10 same-kind findings into a single rollup row (deleted source file, renamed module, entire package tree removed), carry that rollup through to the matching severity table as one row — do not re-expand it here. Rollup rows replace the per-finding `Finding` column with `Root Cause` + `Count` + `Representative symbols` (up to 3 names, `…` if more). Changed-signature and cross-file findings remain per-row; they were not eligible for rollup in step-03 and are not eligible here.

**Rollup row form (any severity table):**

| Root Cause | Count | Representative symbols | Type | Detail | Location | Confidence |
|------------|-------|------------------------|------|--------|----------|------------|
| {root cause} | {N} | `{sym1}`, `{sym2}`, … | {structural/semantic} | {shared detail} | {file}:{line} | {T1/T2} |
