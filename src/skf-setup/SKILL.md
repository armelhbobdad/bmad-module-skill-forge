...
| **Failure modes** | `--require-tier` not satisfied → status becomes `tier_failure`, the envelope sets `"require_tier_satisfied": false`, and the workflow halts before step 5 (interactive runs also print a "REQUIRED TIER NOT MET" block). Write failure (forge-tier.yaml or preferences.yaml could not be written) → status becomes `write_failure`, the envelope `error` field names the path and reason. |
